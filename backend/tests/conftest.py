...
WHY THIS EXISTS
---------------
``_init_state`` calls ``load_dotenv()`` before reading ``ANNEAL_DATABASE_URL``
...
tests gate on their own opt-in variable (``ANNEAL_TEST_DATABASE_URL``).
...
def _never_touch_the_real_database(monkeypatch):
    """Neutralise ANNEAL_DATABASE_URL for every test (autouse, no opt-out).
