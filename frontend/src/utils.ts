...
// NOTE — there is deliberately NO `rulingVerdict` here. Which verdict RULES a
// claim (retracted dropped / CONFIRMED only / last one wins) is the backend
// `verdict_precedent` projection's rule and its alone; the 判例徽章 reads it
// over `GET /claim/{id}/precedent`. A second implementation on this side would
// be free to drift from the confirm gate — the one thing that must not.
