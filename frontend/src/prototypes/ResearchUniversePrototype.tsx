      ...
      <main className="ru-terrain">
        ...
        <section className="ru-direction-field">
          {directions.map((direction, index) => (
            <article key={direction.title} className={`ru-direction ${index === 0 ? "ru-direction-main" : ""}`}>
              ...
              <button className="ru-enter">进入方向 <span>→</span></button>
            </article>
          ))}
          <article className="ru-unplaced"><span>未归属的探索</span><strong>为什么不同的透明机制产生相反结果？</strong><p>它可以先独立存在。</p><button>继续探索 →</button></article>
        </section>
      </main>
      <QuenchPulse />
    </div>
...
export default function ResearchUniversePrototype({ variant }: { variant: Variant }) {
  ...
  return <><style>{styles}</style>{variant === "A" ? <VariantA mode={readingMode} onMode={setReadingMode} /> : variant === "B" ? <VariantB confirmed={evidenceConfirmed} onConfirm={() => setEvidenceConfirmed((value) => !value)} /> : <VariantC />}<Switcher current={variant} /></>
}
