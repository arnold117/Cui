...
function VariantD() {
  const [stage, setStage] = useState<"park" | "release" | "workspace" | "forge" | "grill" | "challenge" | "answered" | "verdict" | "evidence" | "confirmed" | "universe">("park")
  ...
  return <div className="ru-page ru-park-page">
    ...
    <main className="ru-park-main">
      ...
      {stage === "forge" && <section className="ru-d-forge"><article><div className="ru-release-link"><button onClick={() => setStage("park")}>◇ PARK 原件</button><span>→ 放行引用</span></div><p className="ru-eyebrow">原始探索片段</p><blockquote>“在资源稀缺、且历史成绩是主要输入时，课程分配模型会使已有优势更难逆转。”</blockquote><p>原件和探索稿不会被改写。你只是在决定是否亲自形成一条可检验的 claim。</p></article><aside className="ru-focus-pane"><p className="ru-eyebrow">锻 · 形成 claim</p><h2>让它成为可被检验的断言。</h2><p>你究竟在断言什么？</p><textarea defaultValue="在资源稀缺且历史成绩是主要输入时，课程分配模型会使已有优势更难逆转。" /><p>它在哪些条件下不成立？</p><textarea placeholder="由你自己写下范围或可能反例……" /><div className="ru-forge-boundary">只问不给 · Cui 可以澄清，但不会代写候选主张。</div><footer><button onClick={() => setStage("workspace")}>回到探索</button><button className="ru-dark-action" onClick={() => setStage("grill")}>确认提交检验</button></footer></aside></section>}
      {stage === "grill" && <section className="ru-d-forge"><article><div className="ru-release-link"><button onClick={() => setStage("park")}>◇ PARK 原件</button><span>→ 放行引用 → Claim</span></div><p className="ru-eyebrow">已提交的 claim</p><blockquote>“在资源稀缺且历史成绩是主要输入时，课程分配模型会使已有优势更难逆转。”</blockquote><p><QuenchMark label="已提交检验" /> 原始捕获仍在 PARK；这是用户自己确认的独立 claim。</p></article><aside className="ru-focus-pane"><p className="ru-eyebrow">审查轮次 01</p><h2>先看审查地形。</h2><div className="ru-attack-list"><button className="is-active">替代解释是否被排除？</button><button>证据能否区分解释？</button><button>适用边界在哪里？</button></div><p>从替代解释开始：X 与 Y 同时出现，也可能都由 Z 导致。这是必须越过的门槛，不是预判死亡。</p><footer><button onClick={() => setStage("forge")}>回到形成</button><button className="ru-dark-action" onClick={() => setStage("challenge")}>进入这条挑战</button></footer></aside></section>}
      {stage === "challenge" && <section className="ru-d-forge"><article><div className="ru-release-link"><button onClick={() => setStage("park")}>◇ PARK 原件</button><span>→ 放行引用 → Claim → 审查轮次 01</span></div><p className="ru-eyebrow">这条 claim 正在被检验</p><blockquote>“在资源稀缺且历史成绩是主要输入时，课程分配模型会使已有优势更难逆转。”</blockquote><p><QuenchMark label="已提交检验" /> 当前攻击面：替代解释。它要求你检验是否存在另一个因素，同时导致历史成绩和后续课程分配。</p></article><aside className="ru-focus-pane"><p className="ru-eyebrow">Challenge · 替代解释</p><h2>你如何排除另一种解释？</h2><p>为什么问：如果 X 与 Y 同时出现，也可能都由 Z 导致。这里不是要你认输，而是要你说清这条 claim 如何可能被证明错。</p><textarea placeholder="写下你目前的回应……" /><div className="ru-forge-boundary">你也可以带入未确认笔记或待取证材料；它们不会自动变成答辩事实。</div><footer><button onClick={() => setStage("grill")}>回到审查地形</button><button className="ru-dark-action" onClick={() => setStage("answered")}>保存答辩</button></footer></aside></section>}
      {stage === "answered" && <section className="ru-d-forge"><article><div className="ru-release-link"><button onClick={() => setStage("park")}>◇ PARK 原件</button><span>→ 放行引用 → Claim → 审查轮次 01</span></div><p className="ru-eyebrow">已记录答辩</p><blockquote>“在资源稀缺且历史成绩是主要输入时，课程分配模型会使已有优势更难逆转。”</blockquote><p><QuenchMark label="答辩已保存" /> 这只记录了你对“替代解释”的回应；它不等于 challenge 已解决，也不等于 claim 已 survived。</p></article><aside className="ru-focus-pane"><p className="ru-eyebrow">审查地形</p><h2>一条回应已进入账本。</h2><div className="ru-ledger"><span>已回应 · 替代解释</span><span>仍待回应 · 证据门槛</span><span>仍待回应 · 适用边界</span></div><p>现在你可以继续进入另一条挑战，也可以在知情的前提下走向裁决。Cui 不替你决定何时够了。</p><footer><button onClick={() => setStage("challenge")}>修改答辩</button><button className="ru-dark-action" onClick={() => setStage("verdict")}>作出本轮裁决</button></footer></aside></section>}
      {stage === "verdict" && <section className="ru-d-forge"><article><div className="ru-release-link"><button onClick={() => setStage("park")}>◇ PARK 原件</button><span>→ 放行引用 → Claim → 审查轮次 01</span></div><p className="ru-eyebrow">裁决台</p><blockquote>“在资源稀缺且历史成绩是主要输入时，课程分配模型会使已有优势更难逆转。”</blockquote><p>这是用户的判断。Cui 只保留本轮发生过什么，不会替你选择结果。</p></article><aside className="ru-focus-pane"><p className="ru-eyebrow">本轮审查账本</p><h2>在知道未决事项的前提下裁决。</h2><div className="ru-ledger"><span>已回应 · 替代解释</span><span>仍待回应 · 证据门槛</span><span>仍待回应 · 适用边界</span><span>带入但未确认 · Paper A</span></div><div className="ru-verdict-choices"><button className="is-selected">暂时站住了</button><button>这条表述不成立</button><button>需要收窄／改写</button><button>现在先不投入</button></div><textarea placeholder="写下你的裁决理由……" /><footer><button onClick={() => setStage("answered")}>回到审查</button><button className="ru-dark-action ru-confirm-local" onClick={() => setStage("evidence")}>确认本轮裁决</button></footer></aside></section>}
      {stage === "evidence" && <section className="ru-d-evidence"><article><div className="ru-release-link"><button onClick={() => setStage("park")}>◇ PARK 原件</button><span>→ 放行引用 → Claim · 本轮暂时站住</span></div><p className="ru-eyebrow">带入的材料</p><blockquote>“When prior achievement is used as the principal allocation feature, students with earlier access to enrichment are disproportionately routed into advanced tracks.”</blockquote><small>Rodriguez et al. · 2024 · p.12 · 不可变摘录与来源定位</small></article><aside className="ru-focus-pane"><p className="ru-eyebrow">Cui 的取证候选 · 未确认</p><h2>它可能构成反证。</h2><p>这段研究显示路径依赖，但没有将算法匹配与人工分配直接比较。它可能削弱机制表述，而不是自动推翻整个 claim。</p><div className="ru-uncertainty">不确定性：研究对象与当前问题的资源稀缺定义并不相同。</div><footer><button onClick={() => setStage("verdict")}>回到裁决</button><button className="ru-dark-action" onClick={() => setStage("confirmed")}>确认是反证</button></footer></aside></section>}
      {stage === "confirmed" && <section className="ru-d-evidence"><article><div className="ru-release-link"><button onClick={() => setStage("park")}>◇ PARK 原件</button><span>→ 放行引用 → Claim → 已确认取证</span></div><p className="ru-eyebrow">事实已落位</p><blockquote>“When prior achievement is used as the principal allocation feature, students with earlier access to enrichment are disproportionately routed into advanced tracks.”</blockquote><p><QuenchMark label="已确认反证" /> 这项取证不会追溯性杀死 claim；它成为下一次审查或方向回看的可追溯事实。</p></article><aside className="ru-focus-pane"><p className="ru-eyebrow">已确认的取证事实</p><h2>它已落在这一轮审查中。</h2><div className="ru-local-receipt"><span className="ru-pulse-dot" />Paper A → Claim · 反证已确认</div><p>现在请回看研究宇宙：确认后的证据和仍待回应的边界，应当在它们真实影响的位置出现。</p><footer><button onClick={() => setStage("evidence")}>查看材料</button><button className="ru-dark-action ru-confirm-local" onClick={() => setStage("universe")}>回看研究宇宙</button></footer></aside></section>}
      {stage === "universe" && <section className="ru-d-universe"><p className="ru-eyebrow">研究宇宙 · 刚才的轨迹</p><h1>它没有变成一条任务。<br />它留下了可回看的位置。</h1><article className="ru-d-direction"><header><span>方向 01</span><QuenchMark label="更新" /></header><h2>算法化分配如何改变教育资源的不平等，以及什么机制能抵消它。</h2><div><section><p className="ru-eyebrow">正在试验的问题</p><strong>{question}</strong><small>本轮暂时站住 · 已确认一条反证 · 仍待回应 2</small></section><section><p className="ru-eyebrow">刚刚落位的事实</p><strong>Paper A 的摘录与 Claim 构成反证</strong><small>确认后沉入本轮审查，而非自动改变 verdict</small></section></div><footer><button onClick={() => setStage("workspace")}>回到问题试验场</button><button onClick={() => setStage("park")}>查看 PARK 原件</button></footer></article></section>}
    </main>
    <PrototypeGuide
      message={stage === "park" ? "这是一条密封捕获。先点「主动放行」，看看原件如何在不被改写的前提下进入一个问题。" : stage === "release" ? "确认这不是移动文件：原件继续留在 PARK，而你只选择它暂时如何进入研究。点「放行到问题试验场」。" : stage === "workspace" ? "现在你在同一张研究桌上探索。点高亮段落中的「接受检验」，主动把自己写出的断言送进锻。" : stage === "forge" ? "左边保留来路，右边只让你自己写 claim。填写后点「确认提交检验」；它会成为独立 claim 并进入审查。" : stage === "grill" ? "你刚提交的 claim 正在接受第一轮检验。点击「进入这条挑战」，看看攻击面如何在不离开研究桌的情况下展开。" : stage === "challenge" ? "现在你在答辩台。写下回应；保存答辩不会自动结束 challenge。" : stage === "answered" ? "答辩已进入审查账本，但还有其他攻击面。点「作出本轮裁决」，看系统如何呈现未决事实但不替你决定。" : stage === "verdict" ? "选择结果并写理由。这里预选“暂时站住了”，但它明确不是正确性证书。确认后带入一份材料取证。" : stage === "evidence" ? "材料、claim 与 Cui 的候选同台。点「确认是反证」；它会成为事实，不会自动杀死 claim。" : stage === "confirmed" ? "反证已经落位。最后点「回看研究宇宙」，检查这段经历是否在方向里保持连续。" : "旅程完成。这里应只呈现你刚刚形成的问题、claim、确认取证与仍待回应的事实。"}
      action={stage === "park" ? "主动放行" : stage === "release" ? "放行到问题试验场" : stage === "workspace" ? "接受检验" : stage === "forge" ? "确认提交检验" : stage === "grill" ? "进入这条挑战" : stage === "challenge" ? "保存答辩" : stage === "answered" ? "作出本轮裁决" : stage === "verdict" ? "确认本轮裁决" : stage === "evidence" ? "确认是反证" : stage === "confirmed" ? "回看研究宇宙" : "重新查看 PARK 原件"}
      onAction={() => setStage(stage === "park" ? "release" : stage === "release" ? "workspace" : stage === "workspace" ? "forge" : stage === "forge" ? "grill" : stage === "grill" ? "challenge" : stage === "challenge" ? "answered" : stage === "answered" ? "verdict" : stage === "verdict" ? "evidence" : stage === "evidence" ? "confirmed" : stage === "confirmed" ? "universe" : "park")}
    />
    <QuenchPulse />
  </div>
}
...
function PrototypeGuide({ message, action, onAction }: { message: string; action: string; onAction: () => void }) {
  return <div className="ru-prototype-guide"><span>原型引导</span><p>{message}</p><button onClick={onAction}>{action} →</button></div>
}
...
.ru-prototype-guide{position:fixed;left:50%;bottom:20px;transform:translateX(-50%);z-index:25;display:flex;align-items:center;gap:11px;max-width:min(720px,calc(100vw - 44px));padding:10px 11px 10px 14px;background:#202523;color:#edf0e9;box-shadow:0 12px 36px rgba(25,35,30,.24);font:11px/1.45 'IBM Plex Sans','Noto Sans SC',sans-serif}.ru-prototype-guide>span{color:#81c6be;font-size:9px;letter-spacing:.1em;text-transform:uppercase;white-space:nowrap}.ru-prototype-guide p{margin:0;flex:1;color:#d6ddd7}.ru-prototype-guide button{border:1px solid #748079;background:transparent;color:#fff;padding:5px 8px;font-size:10px;white-space:nowrap}.ru-prototype-guide button:hover{border-color:#81c6be;color:#81c6be}
...
.ru-d-evidence{width:100%;display:grid;grid-template-columns:minmax(0,1fr) 360px;background:var(--paper);border-top:1px solid #b8c1ba}.ru-d-evidence>article{padding:42px 48px;background:#f0f1ed}.ru-d-evidence article blockquote{font-family:'Charis SIL','Noto Serif SC',serif;font-size:24px;line-height:1.55;margin:20px 0}.ru-d-evidence article>small{font-size:10px;color:var(--muted)}.ru-d-evidence .ru-focus-pane{border-left:1px solid var(--line);box-shadow:none}.ru-d-universe{width:min(820px,100%);padding:40px 45px;background:var(--paper);border-top:2px solid var(--edge)}.ru-d-universe>h1{font-family:'Charis SIL','Noto Serif SC',serif;font-size:37px;line-height:1.2;letter-spacing:-.04em;font-weight:400;margin:13px 0 36px}.ru-d-direction{border-top:1px solid var(--line);padding-top:17px}.ru-d-direction>header{display:flex;justify-content:space-between;font-size:10px;color:var(--muted);letter-spacing:.08em;text-transform:uppercase}.ru-d-direction h2{font-family:'Charis SIL','Noto Serif SC',serif;font-size:25px;font-weight:400;line-height:1.4;margin:16px 0 27px}.ru-d-direction>div{display:grid;grid-template-columns:1fr 1fr;gap:22px}.ru-d-direction section{border-top:1px solid var(--line);padding-top:12px}.ru-d-direction section strong{font-family:'Charis SIL','Noto Serif SC',serif;display:block;font-size:16px;font-weight:400;line-height:1.45;margin:7px 0}.ru-d-direction section small{font-size:10px;line-height:1.5;color:var(--muted)}.ru-d-direction footer{display:flex;justify-content:space-between;margin-top:27px}.ru-d-direction footer button{border:0;border-bottom:1px solid #8f9992;background:none;padding:0;font-size:11px;color:var(--edge)}
@media(max-width:780px){.ru-reading-layout.ru-has-focus{display:block}.ru-focus-pane{border-left:0;border-top:1px solid var(--line)}.ru-topbar{padding:0 16px}.ru-breadcrumb,.ru-prototype-label{display:none}.ru-reading-layout{display:block}.ru-notebook{padding:27px 25px}.ru-rail{display:none}.ru-question-spine{padding:15px 18px}.ru-question-spine small{margin-left:0}.ru-evidence-header,.ru-terrain{padding-left:20px;padding-right:20px}.ru-evidence-board{padding:0;grid-template-columns:1fr}.ru-source-sheet,.ru-claim-sheet{padding:25px}.ru-direction-field{grid-template-columns:1fr}.ru-unplaced{grid-column:auto}.ru-evidence-header h1,.ru-terrain-intro h1{font-size:32px}.ru-switcher{min-width:280px}.ru-topbar .ru-actions{margin-left:auto}.ru-wordmark{min-width:auto}.ru-park-main{padding:28px 20px}.ru-park-surface,.ru-release-surface,.ru-workspace-surface{padding:28px 25px}.ru-park-surface h1{font-size:34px}.ru-release-fields,.ru-d-forge,.ru-d-evidence{grid-template-columns:1fr}.ru-d-forge>article,.ru-d-evidence>article{padding:29px 25px}.ru-d-forge .ru-focus-pane,.ru-d-evidence .ru-focus-pane{border-left:0;border-top:1px solid var(--line)}.ru-d-universe{padding:28px 25px}.ru-d-universe>h1{font-size:30px}.ru-d-direction>div{grid-template-columns:1fr}}
`
